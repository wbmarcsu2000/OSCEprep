...
47. **Round 7 — 18 oddball cases + Practice-mode guided reveal/teach flow.**
    Library grew 50→68: two atypical "zebra" cases per category (e.g. cardiac
    tamponade, Boerhaave, ruptured AAA, DKA-as-abd-pain, long-QT/torsades,
    HOCM, Wernicke, serotonin syndrome, anaphylaxis, methemoglobinemia,
    malaria, TSS, TTP, G6PD, giardiasis, carcinoid, Wilson's, Budd-Chiari).
    Generated by an 18-agent workflow against a frozen schema contract, then
    machine-validated (revealKeys⊆labs+ekg+cxr, maneuver-id integrity, the
    canonical 6-step shape, standard point/domain weights) and load-tested
    through the real adapter+engine (new src/data/__tests__/allcases.test.ts
    drives all 68 to a scored submission). Practice mode is now a guided
    answer→reveal→teach loop: each step gates "Next" until you reveal the
    model-answer comparison + a StepTeaching interstitial drawn from the
    curriculum framework / work-up menu / quick-management + TeachIM chalk
