...
import { NEURO_MCQS, NEURO_MCQ_SYSTEM_ORDER, NEURO_MCQ_SYSTEMS } from "../neuroMcq";

/**
 * Neurology MCQ bank invariants — mirrors the IM/FM bank data tests. Guards
 * well-formedness, id uniqueness, canonical systems, de-duplication WITHIN the
 * Neuro bank, and teaching-field well-formedness. Overlap with the IM/FM banks is
...
describe("Neurology MCQ bank (data)", () => {
  it("is non-empty", () => {
    expect(NEURO_MCQS.length).toBeGreaterThanOrEqual(1);
  });

  it("every question is a well-formed single-best-answer item", () => {
    const ids = NEURO_MCQS.map((q) => q.id);
    expect(new Set(ids).size, "unique ids").toBe(ids.length);
    const systems = new Set<string>(NEURO_MCQ_SYSTEM_ORDER);
    for (const q of NEURO_MCQS) {
  ...
  it("NEURO_MCQ_SYSTEMS lists exactly the systems that have questions, in order", () => {
    const present = NEURO_MCQ_SYSTEM_ORDER.filter((s) => NEURO_MCQS.some((q) => q.system === s));
    expect(NEURO_MCQ_SYSTEMS).toEqual(present);
  });
